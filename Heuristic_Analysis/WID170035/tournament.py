...
from game_agent import aggressive_heuristic, defensive_heuristic, maximizing_win_chances_heuristic, minimizing_losing_chances_heuristic, chances_heuristic, weighted_chances_heuristic, weighted_chances_heuristic_2, defensive_to_offensive, offensive_to_defensive, custom_score
...
def play_round(agents, num_matches):
    ...
    wins = 0.
    total = 0.
    print("Number of rounds: ", NUM_MATCHES)

    print("\nPlaying Matches:")
    print("----------")

    # Loop different agent to compete
    for idx, agent_2 in enumerate(agents[:-1]):
        
        # agent 1 = test agent, agent 2 = opponent
...
def main():
    ...
    # Agent: Random, MM_Null, MM_Open, MM_Improved, AB_Null, AB_Open, AB_Improved
    ...
    test_agents = [
                   ...
                   Agent(CustomPlayer(score_fn=custom_score, **CUSTOM_ARGS), "Yittyn")
                   ]

    print(DESCRIPTION)

    # Loop different test agent
