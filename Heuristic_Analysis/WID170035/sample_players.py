...
def open_move_score(game, player): 
    ...
    # FOR GREEDY CASE:
    # game @ self = updated game @ forecast game (Random - active player)
    # player = before forecast player (Greedy - active player)
    ...
    # return length of player's legal moves after the opposite player's forecast move 
...
class RandomPlayer():
    ...
    def get_move(self, game, legal_moves, time_left):
        ...
        if not legal_moves:
            return (-1, -1)
        # legal_moves not available 
        return legal_moves[randint(0, len(legal_moves) - 1)]
        # simple choose one legal moves
...
class GreedyPlayer():
    ...
    def get_move(self, game, legal_moves, time_left):
        ...
        # legal_moves available
        # once forecast, player will switch, move will increase, but board @ game not update
        # self remain as greedy player

        # get the move which will lead the player to win @ random player have no legal move (after the forecast move made)
        # or the highest marks - most possible legal moves (after [forecast step in the current legal moves list] made) 
...
if __name__ == "__main__":
    ...
    # only update in new board, current board remain same
    ...
    print(game.to_string())
    print("Move history:\n{!s}".format(history))
    print("\nMove count: {}".format(game.move_count))
